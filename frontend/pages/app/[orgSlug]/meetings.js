...
import { formatMeetingTime, isoToLocalDateTime, localDateTimeToIsoPreservingInstant } from '../../../lib/meeting-time.mjs';
...
function MeetingRow({ meeting, onSelect }) {
  ...
  return <li className={`rounded-lg border border-border p-4 ${cancelled ? 'opacity-70' : ''}`}>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium">{meeting.title}</p><p className="mt-1 text-sm text-muted-foreground">{formatMeetingTime(meeting.startAt, meeting.timezone)} – {formatMeetingTime(meeting.endAt, meeting.timezone)}</p></div><span className="rounded-md bg-muted px-2 py-1 text-xs font-medium uppercase tracking-wide">{cancelled ? 'Cancelled' : meeting.status || 'Scheduled'}</span></div>
...
function MeetingsContent({ orgSlug }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [meetingResult, projectResult, memberResult] = await Promise.all([saasApi.listMeetings({}, token), saasApi.listProjects(token), saasApi.listMembers(token)]);
      setMeetings(meetingResult.meetings || []);
      setProjects(projectResult.projects || []);
      setMembers(memberResult.members || []);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Meetings are unavailable.');
    } finally {
      setLoading(false);
    }
  }, [getToken]);
