...
function BillingContent({ orgSlug }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    auth.getToken().then((token) => saasApi.getBillingStatus(token)).then((result) => { if (active) setBilling(result); }).catch((err) => { if (active) setError(err instanceof ApiError ? err.message : 'Billing status is unavailable.'); });
    return () => { active = false; };
  }, [auth, status]);
  return <WorkspaceFrame orgSlug={orgSlug} active="Billing"><div><h2 className="text-2xl font-semibold">Billing</h2><p className="muted mt-1">Plans and limits are enforced by the server for the active organization.</p></div>{status && <p className="muted" role="status">{status}</p>}{error && <p className="text-sm text-destructive" role="alert">{error}</p>}{billing && <Card className="max-w-2xl"><CardHeader><CardTitle className="capitalize">{billing.planKey} plan</CardTitle><CardDescription>Status: {billing.status}</CardDescription></CardHeader><CardContent><dl className="grid gap-3 text-sm sm:grid-cols-2">{Object.entries(billing.entitlements || {}).map(([key, value]) => <div key={key} className="rounded-md border border-border p-3"><dt className="text-muted-foreground">{key.replaceAll('_', ' ')}</dt><dd className="mt-1 font-medium">{String(value)}</dd></div>)}</dl><p className="mt-4 text-xs text-muted-foreground">Stripe checkout and plan changes require server-side Stripe configuration.</p></CardContent></Card>}</WorkspaceFrame>;
}
