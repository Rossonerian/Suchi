...
function NotificationsContent({ orgSlug }) {
  const auth = useAuth(); const { status } = useClerkPageState(auth); const [notifications, setNotifications] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  async function load() { setLoading(true); setError(''); try { const result = await auth.getToken().then((token) => saasApi.listNotifications({}, token)); setNotifications(result.notifications || []); } catch (err) { setError(err instanceof ApiError ? err.message : 'Notifications are unavailable.'); } finally { setLoading(false); } }
  useEffect(() => { if (!status) load(); }, [auth.getToken, status]);
