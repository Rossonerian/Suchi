...
export default function NotificationsScreen() {
  ...
  const query = useQuery({ queryKey: ['mobile-notifications', organization?.id], enabled: Boolean(organization?.id), queryFn: async () => mobileApi.notifications((await getToken()) || '') });
  const read = useMutation({ mutationFn: async (id: string) => mobileApi.markNotificationRead((await getToken()) || '', id), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['mobile-notifications', organization?.id] }) });
